...
content = {
    ...
    'en': u'{{/Merci de ne pas toucher à cette ligne}}\n<!-- Ne rien éditer au-dessus de cette ligne... -->',
            ...
            for title in titles:
                ...
                try:
                    ...
                    translatedMsg = i18n.twtranslate(mySite,
                                                     'globalfuncfr-cleaned')
                    subst = 'subst:' in translatedContent
                    pos = text.find(translatedContent.strip())
                    if text.strip() == translatedContent.strip():
                        pywikibot.output(u'fr.wikivoyage: The sandbox is still clean, no change necessary.')
                    elif subst and sandboxPage.userName() == mySite.loggedInAs():
                        pywikibot.output(u'fr.wikivoyage: The sandbox might be clean, no change necessary.')
                    elif pos <> 0 and not subst:
                        if self.user:
                            endpos = pos + len(translatedContent.strip())
                            if (pos < 0) or (endpos == len(text)):
                                pywikibot.output(u'fr.wikivoyage: The user sandbox is still clean, no change necessary.')
                            else:
                                sandboxPage.put(text[:endpos], translatedMsg)
                                pywikibot.output(u'fr.wikivoyage: Standard content was changed, user sandbox cleaned.')
                        else:
                            sandboxPage.put(translatedContent, translatedMsg)
                            pywikibot.output(u'fr.wikivoyage: Standard content was changed, sandbox cleaned.')
                    else:
